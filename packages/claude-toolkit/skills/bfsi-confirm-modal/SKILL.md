...
- The audit event (paired via `bfsi-audit-action`) includes `mfa_verified: true` in metadata.

### Step 6 — Audit pairing

For state-changing actions, the confirm modal pairs with `useAuditedAction` (see `bfsi-audit-action` skill). The audit event fires AFTER confirmation, not on modal open.
