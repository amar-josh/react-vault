...
description: Reference for handling real-time data feeds (WebSocket / SSE / polling) in a BFSI app without render-storms. Covers batching with requestAnimationFrame, `useSyncExternalStore` for tickers, subscription deduplication, cleanup-on-unmount, and the BFSI-specific patterns (transaction ticker, live balance, OTP delivery, audit-tail). Auto-loads when the user asks about WebSocket / SSE / EventSource / live data / ticker / streaming / push / polling / `useSyncExternalStore`.
---

# BFSI Perf — Real-Time Data

Tickers, balance streams, transaction notifications, OTP delivery, audit-tail dashboards. The naive way of writing these — `socket.on('message', (m) => setState(m))` — calls `setState` once per server message, which is fine at 1 msg/sec and catastrophic at 100 msg/sec.
...
- **Use the variant's query lib's polling** (RTK Query `pollingInterval`, TanStack `refetchInterval`). Both compare structurally and skip the render if equal.
...
For audit-tail dashboards specifically: prefer paging older entries to a fetch (REST), keep the live stream for _new_ events only. Don't replay 10k historical entries through the WebSocket.
...
- **Balance after transfer**: don't wait for the polling tick after a successful mutation. RTK Query: `invalidatesTags: ['Balance']`. TanStack: `queryClient.invalidateQueries({ queryKey: ['balance'] })`. The mutation triggers a fresh fetch; the polling layer is the safety net.
...
- **No PII in WebSocket frames**: if the message includes a PAN / Aadhaar / account number, scrub before storing in the snapshot. Audit any reveal.
