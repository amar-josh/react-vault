...
Pairs with the variant-specific perf skill:

- RTK Query projects → `templates/rtk-query/.claude/skills/perf-tuning/SKILL.md`
- TanStack Query projects → `templates/tanstack-query/.claude/skills/perf-tuning/SKILL.md`
...
## Cost of every BFSI flag

This toolkit's `useAuditedMutation` / `useAuditedAction` wrap every state-changing call with an audit emit. The audit network call is fire-and-forget but the JSON-stringify + scrub still happens on the main thread. For mutations called per-keystroke (autosave drafts), debounce the audit emit to once-per-N-seconds; otherwise the audit pipeline becomes a perf bottleneck of its own.
