...
- **RTK Query**: use `useGetXQuery` with server pagination as the primary fetch, virtualise the _current page_. `useInfiniteQuery` doesn't exist in RTK Query; emulate with manual cursor state.
- **TanStack Query**: use `useInfiniteQuery` paired with `useVirtualizer` + `IntersectionObserver` on the last visible row to trigger `fetchNextPage`. See `templates/tanstack-query/.claude/skills/perf-tuning/SKILL.md` for the wiring.
