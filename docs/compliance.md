...
| 2.x  | Inventory & data classification | `@scope/core/pii` patterns catalog; `<PIIMaskedDisplay>` data-attrs              |
| 3.x  | Logical access control          | `<ProtectedRoute permission="...">` + `<CanAccess>`                              |
| 4.x  | Encryption                      | `@scope/core/encryption` (AES-GCM 256, RSA-OAEP, envelope)                       |
...
| 8.x  | Logging & monitoring            | `@scope/core/audit` AuditClient (batched, PII-scrubbed)                          |
...
| 6.5.7  | XSS                                 | No `dangerouslySetInnerHTML` unsanitised; CSP from `@scope/core/compliance/csp` |
