...
For audit-tail / transaction-history feeds:
...
**Audit pairing** (BFSI requirement): the audit event for `notification.preference.changed` should fire in `onSuccess` (after the server confirms), NOT in `onMutate`. Optimistic UI is local; the audit reflects the durable change.
...
- `refetchOnWindowFocus: false` — default it to false globally; opt-in per query (balance, audit tail).
