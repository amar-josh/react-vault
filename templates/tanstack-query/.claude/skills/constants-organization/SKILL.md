...
Constants are split by purpose. Each kind has one home — don't scatter.
...
└── appConstants.ts      App-wide enums (storage keys, journey types, etc.)
```
...
| A frontend route path        | `src/utils/constants/routeConstants.ts` | `ROUTES.<feature>.<view>`                              |
| A TanStack queryKey          | `src/utils/constants/queryKeys.ts`      | `<feature>Keys.list()` / `<feature>Keys.detail(id)`    |
| A validation regex           | `src/utils/constants/regexConstants.ts` | `<FIELD>_REGEX`                                        |
| A storage key                | `src/utils/constants/appConstants.ts`   | `<PURPOSE>_KEY`                                        |
| An enum value used > 1 place | `src/utils/constants/appConstants.ts`   | `<NAME>` (UPPER_SNAKE)                                 |
...
promote to a factory in `src/utils/constants/queryKeys.ts`:
...
1. Open `src/utils/constants/routeConstants.ts` (create if it doesn't exist yet).
2. Add the path:
...
import { PII_PATTERNS } from '@react-vault/core/pii';
